...
#ifndef FOR_CHECKMAXIMALITY_HASHHASH_H
#define FOR_CHECKMAXIMALITY_HASHHASH_H
#include "for_test.h"
...
namespace HashHash {
    __global__ void build_node_neighbor_multihash(const uint* nodes, const uint node_count, const uint* degrees,
        const uint* neighbors, const uint* neighbors_offset,
        Utils::MultiHash::cudaMultiHashTable_gpu_viewer tables) {
        const uint tid       = threadIdx.x + blockIdx.x * blockDim.x;
        const uint grid_size = blockDim.x * gridDim.x;
        for (uint idx = tid; idx < node_count; idx += grid_size) {
            const uint node = nodes[idx];
            const uint degree = degrees[node];
            const uint* node_neighbors = neighbors + neighbors_offset[node];
            for (uint neighbor_idx = 0; neighbor_idx < degree; ++neighbor_idx) {
                tables.insert(node_neighbors[neighbor_idx], 1, node);
            }
        }
    }

    inline uint next_pow2_or_one(uint value) {
        if (value <= 1) return 1;
        return static_cast<uint>(std::bit_ceil(value));
    }

    struct NodeNeighborMultiHashOwner {
        uint32_t* keys = nullptr;
        int32_t* values = nullptr;
        uint* sizes = nullptr;
        uint* locks = nullptr;
        uint capacity_per_table = 1;
        uint num_tables = 1;

        NodeNeighborMultiHashOwner() = default;

        NodeNeighborMultiHashOwner(const Partition_v3& partition, const std::vector<uint>& host_degrees,
            const uint* device_degrees, const uint* device_neighbors, const uint* device_neighbors_offset,
            cudaStream_t stream) {
            build(partition, host_degrees, device_degrees, device_neighbors, device_neighbors_offset, stream);
        }

        NodeNeighborMultiHashOwner(const NodeNeighborMultiHashOwner&) = delete;
        NodeNeighborMultiHashOwner& operator=(const NodeNeighborMultiHashOwner&) = delete;

        NodeNeighborMultiHashOwner(NodeNeighborMultiHashOwner&& other) noexcept {
            *this = std::move(other);
        }

        NodeNeighborMultiHashOwner& operator=(NodeNeighborMultiHashOwner&& other) noexcept {
            ...
            sizes = other.sizes;
            locks = other.locks;
            capacity_per_table = other.capacity_per_table;
            num_tables = other.num_tables;
            other.keys = nullptr;
            other.values = nullptr;
            other.sizes = nullptr;
            other.locks = nullptr;
            other.capacity_per_table = 1;
            other.num_tables = 1;
            return *this;
        }

        ~NodeNeighborMultiHashOwner() {
            release();
        }

        void release() {
            if (keys) CUDA_ERROR_CHECK(cudaFree(keys));
            if (values) CUDA_ERROR_CHECK(cudaFree(values));
            if (sizes) CUDA_ERROR_CHECK(cudaFree(sizes));
            if (locks) CUDA_ERROR_CHECK(cudaFree(locks));
            keys = nullptr;
            values = nullptr;
            sizes = nullptr;
            locks = nullptr;
            capacity_per_table = 1;
            num_tables = 1;
        }

        void build(const Partition_v3& partition, const std::vector<uint>& host_degrees, const uint* device_degrees,
            const uint* device_neighbors, const uint* device_neighbors_offset, cudaStream_t stream) {
            release();
            if (partition.totalSize == 0) return;

            std::vector<uint> host_nodes(partition.totalSize);
            CUDA_ERROR_CHECK(cudaMemcpyAsync(host_nodes.data(), partition.partition,
                partition.totalSize * sizeof(uint), cudaMemcpyDeviceToHost, stream));
            CUDA_ERROR_CHECK(cudaStreamSynchronize(stream));

            uint max_degree = 1;
            for (const uint node : host_nodes) {
                max_degree = std::max(max_degree, host_degrees[node]);
            }

            capacity_per_table = std::max(1u, static_cast<uint>(std::ceil(static_cast<double>(max_degree) / 0.4)));
            num_tables = next_pow2_or_one(std::max(1u, partition.totalSize * 2));

            CUDA_ERROR_CHECK(cudaMallocAsync(&keys, sizeof(uint32_t) * capacity_per_table * num_tables, stream));
            CUDA_ERROR_CHECK(cudaMallocAsync(&values, sizeof(int32_t) * capacity_per_table * num_tables, stream));
            CUDA_ERROR_CHECK(cudaMallocAsync(&sizes, sizeof(uint) * num_tables, stream));
            CUDA_ERROR_CHECK(cudaMallocAsync(&locks, sizeof(uint) * num_tables, stream));
            CUDA_ERROR_CHECK(cudaMemsetAsync(keys, 0xFF, sizeof(uint32_t) * capacity_per_table * num_tables, stream));
            CUDA_ERROR_CHECK(cudaMemsetAsync(values, 0, sizeof(int32_t) * capacity_per_table * num_tables, stream));
            CUDA_ERROR_CHECK(cudaMemsetAsync(sizes, 0, sizeof(uint) * num_tables, stream));
            CUDA_ERROR_CHECK(cudaMemsetAsync(locks, 0xFF, sizeof(uint) * num_tables, stream));

            const uint grid_dim = std::min(1024u, CALC_GRID_DIM(partition.totalSize, THREADS_PER_BLOCK));
            build_node_neighbor_multihash<<<grid_dim, THREADS_PER_BLOCK, 0, stream>>>(
                partition.partition, partition.totalSize, device_degrees, device_neighbors, device_neighbors_offset,
                get_viewer());
        }

        Utils::MultiHash::cudaMultiHashTable_gpu_viewer get_viewer() const {
            return {keys, values, sizes, locks, capacity_per_table, num_tables};
        }
    };
}
...
__global__ void enumerate_Y_get_earlyStop2(const uint* X, const uint X_length, const uint* y, const uint y_length,
    ...
    Utils::MultiHash::cudaMultiHashTable_gpu_viewer y_neighbor_hashes, const bool* selected_allStates) {
    ...
    for (uint idx = tid; idx < X_length * y_length; idx += grid_size) {
        ...
        uint x_value = X[x_id];
        uint value   = 0;
        if (G_label[x_value] && y_neighbor_hashes.find_key(x_value, y_value)) {
            value += 1;
        }
...
__global__ void enumerateY_to_get_judge_difference_set(const uint* degrees, uint* neighbors,
    ...
    CompareOp op, uint* iterations, bool* selected_allStates, uint state_id, uint* selected_count, uint *intersection,
    const int threshold2, CompareOp op2, Utils::MultiHash::cudaMultiHashTable_gpu_viewer ext_neighbor_hashes) {
    ...
    for (uint idx = tid; idx < totalLength; idx += grid_size) {
        ...
        if (!ext_neighbor_hashes.find_key(y_data, ext_node)) {
...
__global__ void enumerateY_to_get_judge_difference_set(const uint* degrees, uint* neighbors,
    ...
    Utils::MultiHash::cudaMultiHashTable_gpu_viewer ext_neighbor_hashes) {
...
__global__ void enumerateY_to_get_judge_difference_set2(const uint* degrees, const uint* neighbors,
    ...
    for (uint ext_id = group_id; ext_id < ext_length; ext_id += group_num) {
        ...
        for (uint y_idx = inner_id; y_idx < Y_length; y_idx += group_size) {
            ...
            bool flag = !ext_neighbor_hashes.find_key(y_data, ext_node);
            int value = static_cast<int>(g_state_viewer.get_value(y_data));
...
__global__ void enumerateY_to_get_judge_difference_set_for_special_varY_hash(const uint* Y_partitions,
    const uint64_t Y_length, const uint* ext_partitions, const uint64_t ext_length, arrMapTable g_state_viewer,
    const int threshold, const uint k, CompareOp op, uint* iterations, const uint* combinations,
    uint64_t batch_size, const uint* checks, uint* intersection, const int threshold2, const CompareOp op2,
    Utils::MultiHash::cudaMultiHashTable_gpu_viewer ext_neighbor_hashes) {
    if (ext_length == 0 || Y_length == 0) {
        return;
    }
    const uint64_t total_length = static_cast<uint64_t>(ext_length) * batch_size;
    const uint64_t tid = threadIdx.x + blockIdx.x * blockDim.x;
    const uint64_t grid_size = blockDim.x * gridDim.x;
    for (uint64_t idx = tid; idx < total_length; idx += grid_size) {
        ...
        for (uint combination_id = 0; combination_id < k; ++combination_id) {
            ...
            const int value = static_cast<int>(g_state_viewer.get_value(y_data));
            if (Utils::get_judge(value, threshold, op)) {
                atomicAnd(&iterations[state_id * ext_length + ext_inner_idx], 0);
                break;
            }
...
__global__ void precompute_ext_flags_kernel(
    ...
    for (uint i = 0; i < Y_length; ++i) {
        ...
        bool is_neighbor = ext_neighbor_hashes.find_key(y_data, ext_node);
...
__host__ void enumerate_varY_to_get_judge_difference_set_for_special_v6(
    ...
    
    {
        ...
        update_states_batched_kernel_varY<<<gridSize, blockSize>>>(
            ...
            checks, intersection, threshold2, op2, ext_neighbor_hashes,
            d_ext_flags 
        );
    ...
    if (tid / group_size < partition_data.size[start] * batch_size) {
        ...
        for (uint idx = group_id; idx < end; idx += group_num) {
            ...
            for (uint i = inner_id; i < degrees[node]; i += group_size) {
                ...
                if (y_idx != -1) {
                    const uint* selected_combination = shared_combinations + state_id * k;
                    if (k < 32) {
                        selected_match = Utils::sequentialSearch_unrolled(y_idx, selected_combination, k);
                    }
                    else {
                        for (uint combination_idx = 0; combination_idx < k; ++combination_idx) {
                            if (selected_combination[combination_idx] == y_idx) {
                                selected_match = true;
                                break;
                            }
                        }
                ...
                if (y_idx == varY_length + 1 || selected_match) {
                    local_count++;
                    }
            }
            atomicAdd(&counts[state_id * partition_length + nodeIdx], local_count);
...
void CheckMaximality_for_special(uint* device_degrees, uint* device_neighbors, uint* device_neighbors_offset,
    ...
    Utils::MultiHash::cudaMultiHashTable_gpu_viewer ext_p_neighbor_hashes,
    Utils::MultiHash::cudaMultiHashTable_gpu_viewer ext_q_neighbor_hashes,
    ...
    if (ext_q.totalSize > 0) {
        ...
        uint gridDim = CALC_GRID_DIM(ext_q.totalSize * batch_size, THREADS_PER_BLOCK);
        ...
        if (con_Y.totalSize > 0) {
            const uint total_threads = CALC_GRID_DIM(con_Y.totalSize * ext_q.totalSize * batch_size, THREADS_PER_BLOCK);
            enumerateY_to_get_judge_difference_set_for_special<<<total_threads, THREADS_PER_BLOCK, 0, stream>>>(
                device_degrees, device_neighbors, device_neighbors_offset, con_Y.partition, con_Y.totalSize,
                ext_q.partition, ext_q.totalSize, g_state_viewer, (int) X.totalSize + 1 - (int) buffer.k,
                buffer.k, CompareOp::Less, buffer.d_iterations_q, batch_size, buffer.d_isMax, buffer.d_intersection_q,
                temp_num - 1 - static_cast<int>(buffer.k), CompareOp::Less, ext_q_neighbor_hashes);
        }
        if (component_length < temp_num - 1 - static_cast<int>(buffer.k) && var_Y.totalSize > 0) {
            ...
            enumerateY_to_get_judge_difference_set_for_special_varY_hash<<<grid_num, THREADS_PER_BLOCK, 0, stream>>>(
                var_Y.partition, var_Y.totalSize, ext_q.partition, ext_q.totalSize, g_state_viewer,
    ...
    if (ext_p.totalSize > 0) {
        ...
        if (con_Y.totalSize > 0) {
            const uint total_threads = CALC_GRID_DIM(con_Y.totalSize * ext_p.totalSize * batch_size, THREADS_PER_BLOCK);
            enumerateY_to_get_judge_difference_set_for_special<<<total_threads, THREADS_PER_BLOCK, 0, stream>>>(
                device_degrees, device_neighbors, device_neighbors_offset, con_Y.partition, con_Y.totalSize,
                ext_p.partition, ext_p.totalSize, g_state_viewer, (int) X.totalSize + 1 - (int) buffer.k,
                buffer.k, CompareOp::Less, buffer.d_iterations_p, batch_size, buffer.d_isMax, buffer.d_intersection_p,
                temp_num - 1 - static_cast<int>(buffer.k), CompareOp::Less, ext_p_neighbor_hashes);
        }
        if (component_length < temp_num - 1 - static_cast<int>(buffer.k) && var_Y.totalSize > 0) {
            uint64_t totalThreads = ext_p.totalSize * batch_size;
            uint grid_num = CALC_GRID_DIM(totalThreads, THREADS_PER_BLOCK);
            enumerateY_to_get_judge_difference_set_for_special_varY_hash<<<grid_num, THREADS_PER_BLOCK, 0, stream>>>(
                var_Y.partition, var_Y.totalSize, ext_p.partition, ext_p.totalSize, g_state_viewer,
...
bool CheckMaximality_for_Gnerate(uint* device_degrees, uint* device_neighbors, uint* device_neighbors_offset,
    ...
    Partition_v3& ext_q, Utils::MultiHash::cudaMultiHashTable_gpu_viewer ext_p_neighbor_hashes,
    Utils::MultiHash::cudaMultiHashTable_gpu_viewer ext_q_neighbor_hashes, const uint theta, const uint k, cudaStream_t& stream) {
    bool isMax = true;
    if (ext_q.totalSize) {
        uint* intersection;
        ...
        if (host_result2 == 1) {
            ...
            if (con_Y.totalSize > 0) {
                ...
                enumerateY_to_get_judge_difference_set<<<gridNum, THREADS_PER_BLOCK, 0, stream>>>(device_degrees, device_neighbors,
                    ...
                    (int)con_Y.totalSize + (int)cur_Y.totalSize - (int)k, CompareOp::Less, ext_q_neighbor_hashes);
            }
        ...
        if (isMax) {
            ...
            if (con_Y.totalSize > 0) {
                ...
                enumerateY_to_get_judge_difference_set<<<gridDim, THREADS_PER_BLOCK, 0, stream>>>(device_degrees, device_neighbors,
                    ...
                    g_state_viewer, (int)X.totalSize + 1 - (int)k, CompareOp::Less, iterations, intersection,
                    (int)con_Y.totalSize + (int)cur_Y.totalSize - (int)k, CompareOp::Less, ext_p_neighbor_hashes);
...
bool CheckMaximality(uint* device_degrees, uint* device_neighbors, uint* device_neighbors_offset,
    ...
    bool* selected_allStates, uint* selected_count, Utils::HashTable::cudaHashTable3& hash_table,
    Utils::MultiHash::cudaMultiHashTable_gpu_viewer base_cand_neighbor_hashes,
    Utils::MultiHash::cudaMultiHashTable_gpu_viewer varY_neighbor_hashes,
    Utils::MultiHash::cudaMultiHashTable_gpu_viewer ext_p_neighbor_hashes,
    Utils::MultiHash::cudaMultiHashTable_gpu_viewer ext_q_neighbor_hashes,
    ...
    if (base_candY_p.totalSize + var_Y.totalSize > 0) {
        ...
        if (base_candY_p.totalSize > 0) {
            ...
            enumerate_Y_get_earlyStop2<<<gridDim, 256, 0, stream>>>(X.partition, X.totalSize, base_candY_p.partition,
                ...
                g_temp_viewer, delta_G_temps, base_cand_neighbor_hashes, nullptr);
        ...
        if (var_Y.totalSize > 0) {
            ...
            enumerate_Y_get_earlyStop2<<<gridDim, 256, 0, stream>>>(X.partition, X.totalSize, var_Y.partition,
                ...
                static_cast<int>(con_Y.totalSize + base_cur_Y.totalSize + 1 - k), state_id, result2, device_G_label,
                g_temp_viewer, delta_G_temps, varY_neighbor_hashes, selected_allStates);
            CUDA_ERROR_CHECK(cudaMemcpyAsync(&host_result2, result2, sizeof(uint), cudaMemcpyDeviceToHost, stream));
            CUDA_ERROR_CHECK(cudaStreamSynchronize(stream));
            if (host_result2 == 0) {
    ...
    if (ext_q.totalSize) {
        ...
        const uint gridDim = CALC_GRID_DIM(ext_q.totalSize, blockDim);
        any_satisfied_intersection_size_double_results<<<gridDim, blockDim, 0, stream>>>(intersection, ext_q.totalSize,
        ...
        if (isMax) {
            ...
            if (con_Y.totalSize > 0) {
                enumerateY_to_get_judge_difference_set<<<512, 512, 0, stream>>>(device_degrees, device_neighbors,
                    ...
                    CompareOp::Less, ext_p_neighbor_hashes);
            }
            if (base_cur_Y.totalSize > 0) {
                enumerateY_to_get_judge_difference_set<<<512, 512, 0, stream>>>(device_degrees, device_neighbors,
