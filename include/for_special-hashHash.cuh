...
#ifndef FOR_SPECIAL_HASHHASH_H
#define FOR_SPECIAL_HASHHASH_H
#include <output.cuh>
...
bool check_memory_availability_without_hash_tables(
    const uint64_t _batch_size,
    const uint _k,
    const uint _ext_q_size,
    const uint _ext_p_size,
    ...
    if (overflow_detected) {
        if (verbose) {
            std::cout << "--- GPU Memory Check (No Hash Tables) ---" << std::endl;
            std::cout << "Error: Integer overflow detected during memory calculation." << std::endl;
            std::cout << "Memory is INSUFFICIENT." << std::endl;
            std::cout << "-----------------------------------------" << std::endl;
        }
...
void Special_GPU(Partition_v3& X, Partition_v3& var_Y, Partition_v3& con_Y, Partition_v3& ext_p, Partition_v3& ext_q,
    ...
    uint*& device_neighbors, uint*& device_neighbors_offset, const std::vector<uint>& host_degrees,
    const uint component_length, const uint theta, const uint k, bool OutputResults, cudaStream_t& stream) {
    ...
    uint64_t batch_size = totalCombinationNum;
    while (batch_size) {
        if (check_memory_availability_without_hash_tables(
                batch_size, k, ext_q.totalSize, ext_p.totalSize, false, true)) {
            break;
        }
        batch_size /= 2;
    }
    ...
    fmt::println("pure hash-hash version");
    ...
    while (generator.hasMoreCombinations()) {
        ...
        CheckMaximality_for_special(device_degrees, device_neighbors, device_neighbors_offset, device_G_prune,
            combinations, useful_batch, g_state_viewer, hash_table,
            ext_p_neighbor_hashes.get_viewer(), ext_q_neighbor_hashes.get_viewer(), k, X, con_Y,
...
void Special_GPU_optimized(Partition_v3& X, Partition_v3& var_Y, Partition_v3& con_Y, Partition_v3& ext_p, Partition_v3& ext_q,
    arrMapTable& g_state_viewer, bool* device_G_prune, uint*& device_degrees,
    uint*& device_neighbors, uint*& device_neighbors_offset, const std::vector<uint>& host_degrees,
    const uint right_length, const uint theta, const uint k, bool OutputResults, cudaStream_t& stream) {
    OutputFormat_class outputFormat;
    uint* host_varY = nullptr;
