...
#ifndef _FOR_BFS_HASHHASH_CUH_
#define _FOR_BFS_HASHHASH_CUH_
#include <for_checkmaximality-hashHash.cuh>
#include <for_special-hashHash.cuh>
...
__global__ void check_states_by_candidates_blockwise(const uint* __restrict__ varY, uint varY_length,
    ...
    for (uint xIdx = threadIdx.x; xIdx < X_length; xIdx += blockDim.x) {
        ...
        if (G_label[x] == 1) {
            if (varY_neighbor_hashes.find_key(x, u)) {
                ++val;
            }
        }
...
void ListBFS2_GPU(Partition_v3& X, Partition_v3& var_Y, Partition_v3& con_Y, Partition_v3& cur_Y, Partition_v3& candY_q,
    ...
    const std::vector<uint>& host_degrees, const uint theta, const uint k, cudaStream_t& stream) {
    ...
    auto varY_neighbor_hashes = HashHash::NodeNeighborMultiHashOwner(
        var_Y, host_degrees, device_degrees, device_neighbors, device_neighbors_offset, stream);
    auto candY_q_neighbor_hashes = HashHash::NodeNeighborMultiHashOwner(
        candY_q, host_degrees, device_degrees, device_neighbors, device_neighbors_offset, stream);
    auto ext_p_neighbor_hashes = HashHash::NodeNeighborMultiHashOwner(
        ext_p, host_degrees, device_degrees, device_neighbors, device_neighbors_offset, stream);
    auto ext_q_neighbor_hashes = HashHash::NodeNeighborMultiHashOwner(
        ext_q, host_degrees, device_degrees, device_neighbors, device_neighbors_offset, stream);
        ...
        if (current_level >= var_Y.totalSize) {
            ...
            if (satisfied_state_num > 0) {
                ...
                check_states_by_candidates_blockwise<<<grid, block, 0, stream>>>(var_Y.partition, var_Y.totalSize,
                    ...
                    g_temp_viewer, delta_G_temps, varY_neighbor_hashes.get_viewer(), current_level_states,
                ...
                for (uint _state_id = 0; _state_id < current_state_num; _state_id++) {
                    if (can_extends[_state_id] == 0) {
                        bool res = CheckMaximality(device_degrees, device_neighbors, device_neighbors_offset, delta_G_temps, device_G_prune, g_temp_viewer, g_state_viewer,
                            ...
                            current_level_states.selected_allStates, select_counts, hash_table,
                            candY_q_neighbor_hashes.get_viewer(), varY_neighbor_hashes.get_viewer(),
                            ext_p_neighbor_hashes.get_viewer(), ext_q_neighbor_hashes.get_viewer(), theta, k, stream);
